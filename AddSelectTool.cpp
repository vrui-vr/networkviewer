/***********************************************************************
AddSelectTool - Tool class to add single nodes from a network to the
current selection
...
/*****************************************************
Static elements of class NetworkViewer::AddSelectTool:
*****************************************************/

NetworkViewer::AddSelectTool::Factory* NetworkViewer::AddSelectTool::factory=0;

/*********************************************
Methods of class NetworkViewer::AddSelectTool:
*********************************************/

void NetworkViewer::AddSelectTool::initClass(void)
	{
	/* Create a factory object for the custom tool class: */
	factory=new Factory("AddSelectTool","Select Nodes",Tool::factory,*Vrui::getToolManager());
...
NetworkViewer::AddSelectTool::AddSelectTool(const Vrui::ToolFactory* factory,const Vrui::ToolInputAssignment& inputAssignment)
	:Tool(factory,inputAssignment)
	{
	}

const Vrui::ToolFactory* NetworkViewer::AddSelectTool::getFactory(void) const
	{
	return factory;
	}

void NetworkViewer::AddSelectTool::buttonCallback(int buttonSlotIndex,Vrui::InputDevice::ButtonCallbackData* cbData)
	{
	if(cbData->newButtonState) // Button has just been pressed
		...
		if(pickedNodeIndex!=~0x0U&&application->lockSelection())
			...
			application->network->selectNode(pickedNodeIndex);
